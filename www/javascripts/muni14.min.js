var utils={addSelectOption:function(selectbox,value,text){var optn=document.createElement("OPTION");optn.text=text;optn.value=value;selectbox.options.add(optn)},clearSelect:function(selectbox){selectbox.innerHTML=""},insertAfter:function(newNode,nodeRef){nodeRef.parentNode.insertBefore(newNode,nodeRef.nextSibling)}};var muni14={data:[
	...
	{dept:"Retalhuleu",
		...
		{dept:"Suchitepéquez",
		munis:["San Juan Bautista","Río Bravo","San José El Idolo","Santo Domingo Suchitepéquez","San Gabriel","San Miguel Panán","San Bernandino","Patulul","San Antonio Suchitepéquez","Santa Bárbara","Mazatenango","Samayac","Cuyotenango","San Pablo Jocopila","Santo Tomas La Unión","Chicacao","Zunilito","San Francisco Zapotitlán","San Francisco Zapotitlán","Pueblo Nuevo","San Lorenzo"]},
		...
		{dept:"Sololá",
		munis:["Sololá","Nahualá","Santa Catarina Ixtahuacán","Santa Lucía Utatlán","San Andrés Semetabaj","Concepción","San José Chacayá","Panajachel","Santa Cruz La Laguna","San Marcos La Laguna","Lago de Atitlán","San Pablo La Laguna","Santa Clara La Laguna","Santa Catarina Palapó","Santa María Visitación","San Juan La Laguna","San Antonio Palapó","San Pedro La Laguna","Santiago Atitlán","San Lucas Tolimán",""]}
	
		
		],addDepts:function(selectDeptId){var selectDept=document.getElementById(selectDeptId),deps=[];for(var i=0,t=this.data.length;i<t;i++){deps.push(this.data[i].dept)}deps.sort();for(var j=0,t2=deps.length;j<t2;j++){utils.addSelectOption(selectDept,this.getDeptCode(deps[j]),deps[j])}var selectMunis=document.createElement("select");selectMunis.setAttribute("id",selectDept.getAttribute("id")+"-munis");selectMunis.setAttribute("class","form-control");selectMunis.setAttribute("ng-model","municipio");selectMunis.setAttribute("data-error","Campo Requerido");selectMunisCachedDisplay=selectMunis.style.display;selectMunis.style.display="none";selectMunis.required=true;utils.insertAfter(selectMunis,selectDept);var that=this;selectDept.onchange=function(){var opt=this.value;if(opt>0){utils.clearSelect(selectMunis);utils.addSelectOption(selectMunis," "," ");var munis=that.data[--opt].munis;for(var i=0,t=munis.length;i<t;i++){var code=i+1;utils.addSelectOption(selectMunis,code>9?code:"0"+code,munis[i]);selectMunis.style.display=selectMunisCachedDisplay}}else{utils.clearSelect(selectMunis);selectMunis.style.display="none"}}},getDept:function(id){if(typeof id==="string"){id=parseInt(id,10)}return(id>0&&id<15)?this.data[--id].dept:""},getMunis:function(deptId){return this.getDept(deptId)!==""?this.data[--deptId].munis:""},getDeptCode:function(dept){for(var i=0,t=this.data.length;i<t;i++){if(this.data[i].dept===dept){var code=++i;return code>9?code:"0"+code}}},getDeptMuni:function(code){if(code.match(/\d{4}/)){var deptCode=code.substring(0,2),muniCode=parseInt(code.substring(2),10)-1,dept=muni14.getDept(deptCode);if(dept!==""&&muniCode>=0){var munis=this.getMunis(deptCode);return muniCode<munis.length?dept+", "+munis[muniCode]:""}}}};